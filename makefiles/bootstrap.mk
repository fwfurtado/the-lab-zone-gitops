...
bootstrap-seal-infisical-secrets:
	...
	$(KUBECTL) -n infisical create secret generic infisical-secrets \
		...
		--from-literal=DB_CONNECTION_URI="postgresql://infisical:$${INFISICAL_DB_PASSWORD}@postgres.database.svc.cluster.local:5432/infisicalDB" \
...
bootstrap-seal-coder-db-url:
	...
	$(KUBECTL) -n coder create secret generic coder-db-url \
		--from-literal=url="postgres://coder:$${CODER_PW}@postgres.database.svc.cluster.local:5432/coder?sslmode=disable" \
		--dry-run=client -o yaml \
	| kubeseal --format yaml --cert $$tmp_cert \
